...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    private void initializeTrustVerificationCollection() {
        logger.info("Initializing trust_verifications collection...");

        boolean exists = mongoTemplate.collectionExists(com.urlshortener.model.TrustVerification.class);

        if (!exists) {
            logger.info("Creating trust_verifications collection...");
            mongoTemplate.createCollection(com.urlshortener.model.TrustVerification.class);
        } else {
            logger.info("Trust Verifications collection already exists");
        }

        IndexOperations indexOps = mongoTemplate.indexOps(com.urlshortener.model.TrustVerification.class);

        try {
            // 1. Unique index on userId (One application per user)
            indexOps.ensureIndex(
                    new Index()
                            .on("userId", org.springframework.data.domain.Sort.Direction.ASC)
                            .unique()
                            .named("idx_trust_userid_unique")
                            .background());
            ...
            indexOps.ensureIndex(
                    new Index()
                            .on("status", org.springframework.data.domain.Sort.Direction.ASC)
                            .named("idx_trust_status")
                            .background());

            logger.info("Trust Verifications indexes created");
