...
const DomainTransferModal: React.FC<DomainTransferModalProps> = ({
  ...
  const transferMutation = useMutation({
    mutationFn: (newOwnerId: string) => 
      adminApiEndpoints.domains.transfer(domain!.id, newOwnerId),
    onSuccess: () => {
      toast.success('Domain transferred successfully');
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Failed to transfer domain');
    },
  });
  ...
  const isConfirmValid = confirmText === domain?.domain;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center p-4 bg-warning-50 dark:bg-warning-900/20 rounded-lg">
        ...
        <div>
          ...
          <p className="text-sm text-warning-700 dark:text-warning-300">
            This action will transfer the domain to another user. All associated links will be transferred as well.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div className="w-10 h-10 bg-primary-100 dark:bg-primary-900 rounded-full flex items-center justify-center mr-3">
            <span className="text-primary-600 dark:text-primary-400 font-medium">
              {domain.owner?.name?.charAt(0).toUpperCase()}
            </span>
          </div>
          <div>
          ...
          <div className="relative">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search for a user to transfer domain to..."
            ...
            {userSearch.length > 2 && (
              <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                {searchingUsers ? (
                  ...
                  searchResults
                    .filter((user: User) => user.id !== domain.owner?.id)
                    .map((user: User) => (
                      <button
                        key={user.id}
                        type="button"
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          Type <strong>{domain.domain}</strong> to confirm the domain transfer:
        </p>
        <input
          type="text"
          value={confirmText}
