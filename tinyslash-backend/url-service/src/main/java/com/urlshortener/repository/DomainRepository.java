...
public interface DomainRepository extends MongoRepository<Domain, String> {
    ...
    // Find domains stuck in SSL provisioning (Verified but SSL Pending > threshold)
    @Query("{'status': 'VERIFIED', 'sslStatus': {$in: ['PENDING', 'ERROR']}, 'updatedAt': {$lt: ?0}}")
    List<Domain> findStuckSslDomains(LocalDateTime threshold);

    // Find domains pending deletion
