...
public class PerformanceMonitoringService {
    ...
    public Map<String, String> getPerformanceRecommendations() {
        ...
        try {
            ...
            if (avgApiTime > 500) {
                recommendations.put("api",
                        "API response times are high. Consider optimizing queries or adding more caching");
            }

            double avgDbTime = getAverageOperationTime("db.query");
            if (avgDbTime > 200) {
                recommendations.put("database",
    ...
    private void recordOperationTime(String operation, long timeMs) {
        ...
        operationTimes.entrySet().removeIf(entry -> {
            ...
            return System.currentTimeMillis() - Long.parseLong(parts[parts.length - 1]) > 300000;
        }); // 5 minutes
    }
